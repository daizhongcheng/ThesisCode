function varargout = Parameter_layer5(varargin)
% PARAMETER_LAYER5 MATLAB code for Parameter_layer5.fig
%      PARAMETER_LAYER5, by itself, creates a new PARAMETER_LAYER5 or raises the existing
%      singleton*.
%
%      H = PARAMETER_LAYER5 returns the handle to a new PARAMETER_LAYER5 or the handle to
%      the existing singleton*.
%
%      PARAMETER_LAYER5('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PARAMETER_LAYER5.M with the given input arguments.
%
%      PARAMETER_LAYER5('Property','Value',...) creates a new PARAMETER_LAYER5 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Parameter_layer5_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Parameter_layer5_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Parameter_layer5

% Last Modified by GUIDE v2.5 08-Feb-2017 14:12:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Parameter_layer5_OpeningFcn, ...
                   'gui_OutputFcn',  @Parameter_layer5_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Parameter_layer5 is made visible.
function Parameter_layer5_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Parameter_layer5 (see VARARGIN)

% Choose default command line output for Parameter_layer5
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Parameter_layer5 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Parameter_layer5_OutputFcn(hObject, eventdata, handles) 
...
load('fileData.mat');
    if cause1 ==1
        f_parameter.Cause_Nr(1,5) = 1;
    elseif cause2 ==1
        f_parameter.Cause_Nr(1,5) = 2;
    elseif cause3 ==1
        f_parameter.Cause_Nr(1,5) = 3;
    end
    
    %which functions are chosen
    f_parameter.Fault_function(:,:,5) = [p11 p12 p13;p21 p22 p23;p31 p32 p33];
%save function parameters
    f_parameter.Fault_range(:,:,5) = [phi11min phi11max phi12min phi12max phi13min phi13max;phi21min phi21max phi22min phi22max phi23min phi23max;,...
                          phi31min phi31max phi32min phi32max phi33min phi33max];
%save data to mat. file    
    %recheck hidden_layer
    if (p11+p12+p13)==0
        f_parameter.Hidden_pro(1,:,5) = zeros(1,3);
    elseif (p11+p12+p13)==1
        f_parameter.Hidden_pro(1,:,5) = [0 1 0];
    elseif (p21+p22+p23)==0
        f_parameter.Hidden_pro(2,:,5) = zeros(1,3);
    elseif (p21+p22+p23)==1
        f_parameter.Hidden_pro(2,:,5) = [0 1 0];
    elseif (p31+p32+p33)==0
        f_parameter.Hidden_pro(3,:,5) = zeros(1,3);
    elseif (p31+p32+p33)==1
        f_parameter.Hidden_pro(3,:,5) = [0 1 0];
    end
