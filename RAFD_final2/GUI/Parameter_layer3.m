function varargout = Parameter_layer3(varargin)
% PARAMETER_LAYER3 MATLAB code for Parameter_layer3.fig
%      PARAMETER_LAYER3, by itself, creates a new PARAMETER_LAYER3 or raises the existing
%      singleton*.
%
%      H = PARAMETER_LAYER3 returns the handle to a new PARAMETER_LAYER3 or the handle to
%      the existing singleton*.
%
%      PARAMETER_LAYER3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PARAMETER_LAYER3.M with the given input arguments.
%
%      PARAMETER_LAYER3('Property','Value',...) creates a new PARAMETER_LAYER3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Parameter_layer3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Parameter_layer3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Parameter_layer3

% Last Modified by GUIDE v2.5 27-Apr-2017 11:57:43

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Parameter_layer3_OpeningFcn, ...
                   'gui_OutputFcn',  @Parameter_layer3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Parameter_layer3 is made visible.
function Parameter_layer3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Parameter_layer3 (see VARARGIN)

% Choose default command line output for Parameter_layer3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Parameter_layer3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Parameter_layer3_OutputFcn(hObject, eventdata, handles) 
...
if range_f ==1
    ...
    if range_s ==1
        c=questdlg('Do you want to set an exact number for your range?','Close','Yes','No','Yes');  
        if strcmp(c,'Yes')    
%save cause number
load('fileData.mat');
    if cause1 ==1
        f_parameter.Cause_Nr(1,3) = 1;
    elseif cause2 ==1
        f_parameter.Cause_Nr(1,3) = 2;
    elseif cause3 ==1
        f_parameter.Cause_Nr(1,3) = 3;
    end
    
    %which functions are chosen
    f_parameter.Fault_function(:,:,3) = [p11 p12 p13;p21 p22 p23;p31 p32 p33];
%save function parameters
    f_parameter.Fault_range(:,:,3) = [phi11min phi11max phi12min phi12max phi13min phi13max;phi21min phi21max phi22min phi22max phi23min phi23max;,...
                          phi31min phi31max phi32min phi32max phi33min phi33max];
%save data to mat. file    
    %recheck hidden_layer
    if (p11+p12+p13)==0
        f_parameter.Hidden_pro(1,:,3) = zeros(1,3);
    elseif (p11+p12+p13)==1
        f_parameter.Hidden_pro(1,:,3) = [0 1 0];
    end
    if (p21+p22+p23)==0
        f_parameter.Hidden_pro(2,:,3) = zeros(1,3);
    elseif (p21+p22+p23)==1
        f_parameter.Hidden_pro(2,:,3) = [0 1 0];
    end
    if (p31+p32+p33)==0
        f_parameter.Hidden_pro(3,:,3) = zeros(1,3);
    elseif (p31+p32+p33)==1
        f_parameter.Hidden_pro(3,:,3) = [0 1 0];
    end
save ('fileData','f_parameter');
    if Fault_Number > 3
        load('fileData.mat');
        f_parameter.Hidden_pro(:,:,4) = zeros(3,3);
        save ('fileData','f_parameter');
        FaultNr_now = 4;
        close;
        Parameter_layer4;
...
%save data to mat. file    
    ...
    if (p11+p12+p13)==0
        f_parameter.Hidden_pro(1,:,3) = zeros(1,3);
    elseif (p11+p12+p13)==1
        f_parameter.Hidden_pro(1,:,3) = [0 100 0];
    end
    if (p21+p22+p23)==0
        f_parameter.Hidden_pro(2,:,3) = zeros(1,3);
    elseif (p21+p22+p23)==1
        f_parameter.Hidden_pro(2,:,3) = [0 100 0];
    end
    if (p31+p32+p33)==0
        f_parameter.Hidden_pro(3,:,3) = zeros(1,3);
    elseif (p31+p32+p33)==1
        f_parameter.Hidden_pro(3,:,3) = [0 100 0];
    end
save ('fileData','f_parameter');
    if Fault_Number > 3
        load('fileData.mat');
