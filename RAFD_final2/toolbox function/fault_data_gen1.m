function [fault_f,Pro_f] = fault_data_gen1(f_parameter,c_parameter,k_stop,Ts)
 %%%%%%%%%%
 ...
 %%
 %%%%%%% this function is used for the selection of save fault data. it is
 %%%%%%% used for to generate the fault data, which is not added to the
 %%%%%%% original data yet.

% Call:
%         [fault_f,Pro_f] = fault_data_gen_save(f_parameter,Sample_Size,k_stop)
% 
% Inputs: 
...
%         
% Outputs:
%          fault_f: each fault combination-- t relate fault data
%          Pro_f:   probability of each fault combination
%         
% Notes: NONE
%
...
t=0:Ts:(k_stop-1)*Ts;
%t=t(1:k_stop);
for fault_number=1:faultnumber  % each pattern, at most there are 3 fault
    if any(Fault_Pattern(:,fault_number))% to check if the fault exists, exist: 1
