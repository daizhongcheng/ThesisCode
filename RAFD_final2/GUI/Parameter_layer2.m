function varargout = Parameter_layer2(varargin)
% PARAMETER_LAYER2 MATLAB code for Parameter_layer2.fig
%      PARAMETER_LAYER2, by itself, creates a new PARAMETER_LAYER2 or raises the existing
%      singleton*.
%
%      H = PARAMETER_LAYER2 returns the handle to a new PARAMETER_LAYER2 or the handle to
%      the existing singleton*.
%
%      PARAMETER_LAYER2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PARAMETER_LAYER2.M with the given input arguments.
%
%      PARAMETER_LAYER2('Property','Value',...) creates a new PARAMETER_LAYER2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Parameter_layer2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Parameter_layer2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Parameter_layer2

% Last Modified by GUIDE v2.5 27-Apr-2017 13:46:59

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Parameter_layer2_OpeningFcn, ...
                   'gui_OutputFcn',  @Parameter_layer2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Parameter_layer2 is made visible.
function Parameter_layer2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Parameter_layer2 (see VARARGIN)

% Choose default command line output for Parameter_layer2
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Parameter_layer2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Parameter_layer2_OutputFcn(hObject, eventdata, handles) 
...
load('fileData.mat');
    if cause1 ==1
        f_parameter.Cause_Nr(1,2) = 1;
    elseif cause2 ==1
        f_parameter.Cause_Nr(1,2) = 2;
    elseif cause3 ==1
        f_parameter.Cause_Nr(1,2) = 3;
    end
    
    %which functions are chosen
    f_parameter.Fault_function(:,:,2) = [p11 p12 p13;p21 p22 p23;p31 p32 p33];
%save function parameters
    f_parameter.Fault_range(:,:,2) = [phi11min phi11max phi12min phi12max phi13min phi13max;phi21min phi21max phi22min phi22max phi23min phi23max;,...
                          phi31min phi31max phi32min phi32max phi33min phi33max];
%save data to mat. file    
    %recheck hidden_layer
    if (p11+p12+p13)==0
        f_parameter.Hidden_pro(1,:,2) = zeros(1,3);
    elseif (p11+p12+p13)==1
        f_parameter.Hidden_pro(1,:,2) = [0 1 0];
    end
    if (p21+p22+p23)==0
        f_parameter.Hidden_pro(2,:,2) = zeros(1,3);
    elseif (p21+p22+p23)==1
        f_parameter.Hidden_pro(2,:,2) = [0 1 0];
    end
    if (p31+p32+p33)==0
        f_parameter.Hidden_pro(3,:,2) = zeros(1,3);
    elseif (p31+p32+p33)==1
        f_parameter.Hidden_pro(3,:,2) = [0 1 0];
    end  
save ('fileData','f_parameter');

    if Fault_Number > 2
        load('fileData.mat');
        f_parameter.Hidden_pro(:,:,3) = zeros(3,3);
        save ('fileData','f_parameter');
        FaultNr_now = 3;
        close;
        Parameter_layer3;
...
%save data to mat. file    
    ...
    if (p11+p12+p13)==0
        f_parameter.Hidden_pro(1,:,2) = zeros(1,3);
    elseif (p11+p12+p13)==1
        f_parameter.Hidden_pro(1,:,2) = [0 100 0];
    end
    if (p21+p22+p23)==0
        f_parameter.Hidden_pro(2,:,2) = zeros(1,3);
    elseif (p21+p22+p23)==1
        f_parameter.Hidden_pro(2,:,2) = [0 100 0];
    end
    if (p31+p32+p33)==0
        f_parameter.Hidden_pro(3,:,2) = zeros(1,3);
    elseif (p31+p32+p33)==1
        f_parameter.Hidden_pro(3,:,2) = [0 100 0];
    end  
save ('fileData','f_parameter');
