function [A,B,C,D] = random_Sys_d( N,ni,no,lamda )
%   Function 'rondom_Sys_d' is to generate random discrete system which
...
%   for example: rondom_Sys_d( N ) then the default value is 
%   ni=1, no=1 and lamda=1
%
%   When there are two input variables 
%   for example: rondom_Sys_d( N,ni ) then the default value is 
%   no=1 and lamda=1
%
%   When there are three input variables 
%   for example: rondom_Sys_d( N,ni,no ) then the default value is 
%   lamda=1
%
...
%   for example: rondom_Sys_d( N,ni,no,lamda )
%
%
%   Version 1.1
%
...
if mod(N,2) == 0
    ...
    l(round(N/2)) = (2*rand-1);
end
       
% l = (2*rand(N,1)-1) + (2*rand(N,1)-1)*1j;
L = diag(l);
A = U*L*U';
% % w = rand;
% % alpha = w^( 1/ (N^2) );
